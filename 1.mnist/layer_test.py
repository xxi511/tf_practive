...
def main(_):
    ...
    res_conv1 = layers.conv2d(x_image, 32, 5, 'conv1', activation=tf.nn.relu)
    res_pool1 = max_pool_2x2(res_conv1)

    res_conv2 = layers.conv2d(res_pool1, 64, 5, 'conv2', activation=tf.nn.relu)
    res_pool2 = max_pool_2x2(res_conv2)
    flat_pool2 = tf.reshape(res_pool2, [-1, 7*7*64])

    res_fc3 = layers.fully_connected(flat_pool2, 1024, 'fc3', tf.nn.relu)
    res_fc3 = tf.nn.dropout(res_fc3, keep_prob)

    y = layers.fully_connected(res_fc3, 10, 'fc4')
